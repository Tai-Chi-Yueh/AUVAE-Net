...
hsi = load_HSI(
    "C:/Users/10070/Desktop/HsU/Datasets/" + datasetnames[dataset] + ".mat")
x = hsi.array()
E = hsi.gt
end = []
abu = []
...
# activation_set 是激活函数，使用 LeakyReLU。
...
def create_model(input_dim, latent_dim):
    ...
    # 编码器采用 4 层 Dense（全连接层），维度依次递减：9631*端元数
    encoder = Sequential()
    ...
    # SparseReLU 进行稀疏约束，SumToOne 确保丰度和为 1（ASC 约束）。

    # 解码器包含一个全连接层，将 latent_dim 维的丰度向量解码为 input_dim 维的光谱数据。
    decoder = Sequential()
    ...
    # discriminator: 监督 encoder 生成合理的丰度分布
    autoencoder = Model(autoencoder_input, decoder(encoder(autoencoder_input)))
    autoencoder.compile(optimizer=Adam(learning_rate=2e-4), loss=normSAD)

    discriminator.compile(optimizer=Adam(learning_rate=5e-5), loss="binary_crossentropy")

...
def create_model(input_dim, latent_dim):
    ...
    # stage_1
    conv1 = Conv2D(64, (3, 3), activation=activation_set, padding="same", name='Conv_1')(unet_input)
    conv1 = BatchNormalization()(conv1)
    # stage_2
    conv2 = Conv2D(128, (3, 3), activation=activation_set, padding="same", name='Conv_2')(conv1)
    conv2 = BatchNormalization()(conv2)
    # stage_3
    conv3 = Conv2D(256, (3, 3), activation=activation_set, padding="same", name='Conv_3')(conv2)
    conv3 = BatchNormalization()(conv3)    
    # stage_4
    # Bottleneck layer 瓶颈层
    bottleneck = Conv2D(512, (3, 3), activation=activation_set, padding="same", name='Bottleneck')(conv3)
    bottleneck = Dropout(0.2)(bottleneck)
    
    # U_net 上采样层 Upsampling
    # stage_5
    deconv1 = Conv2D(256, (3, 3), padding='same', activation=activation_set, name='Deconv_1')(bottleneck)
    merge1 = Concatenate(name='Merge_1')([conv3, deconv1])
    # stage_6
    deconv2 = Conv2D(128, (3, 3), padding='same', activation=activation_set, name='Deconv_2')(merge1)
    merge2 = Concatenate(name='Merge_2')([conv2, deconv2])
    # stage_7
    deconv3 = Conv2D(64, (3, 3), padding='same', activation=activation_set, name='Deconv_3')(merge2)
    merge3 = Concatenate(name='Merge_3')([conv1, deconv3])

    # U-Net 的最终输出
    unet_output = Conv2D(input_dim, (1, 1), kernel_regularizer=l2(1e-3), activation=activation_set, padding='same', name='Unet_Output')(merge3)
    # unet_output = Conv2D(input_dim, (1, 1), activation='relu', padding='same', name='Unet_Output')(merge3)
    ...
    # VAE 输出均值mu和方差var，重采样合成z
    mu = Dense(latent_dim, use_bias=use_bias, kernel_initializer=None, activation='linear', name='mu')(encode)
    var = Dense(latent_dim, use_bias=use_bias, kernel_initializer=None, activation='linear', name='var')(encode)
        
    # 批归一化处理
    mu = BatchNormalization()(mu)
    # Soft Thresholding, SparseReLU 进行稀疏约束
    mu = SparseReLU(alpha_initializer='zero', alpha_constraint=non_neg(), activity_regularizer=None)(mu)
    # Sum To One (ASC), SumToOne 确保丰度和为 1（ASC 约束）
    mu = SumToOne(axis=0, name='Abundances', activity_regularizer=None)(mu)
    
    z = Lambda(resampling, output_shape=(latent_dim,), name='z')([mu, var])
    # 封装encoder模型
    encoder = Model(auv_input, mu, name='Encoder')
    ...
    # 计算VAE损失函数 vaeloss
    vaeloss = Lambda(vae_loss, output_shape=(1,), name='vae')([auv_input, Y_hat, mu, var])


    # 封装自编码器模型：U-net + VAE部分  
    autoencoder = Model(auv_input, [Y_hat, vaeloss, mu], name='VAE')
    autoencoder.compile(optimizer=Adam(learning_rate=2e-4), loss=[normSAD, "mae", None], loss_weights=[1,1,0])
